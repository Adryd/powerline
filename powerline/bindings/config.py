...
def source_tmux_files(pl, args, tmux_version=None, source_tmux_file=source_tmux_file):
    ...
    * If files for same versions are to be sourced then first _minus files are
    ...
    source_tmux_file(os.path.join(
        TMUX_CONFIG_DIRECTORY, 'powerline-base.conf'))
...
def init_tmux_environment(pl, args, set_tmux_environment=set_tmux_environment):
    ...
    '''
    powerline = ShellPowerline(finish_args(
        None, os.environ, EmptyArgs('tmux', args.config_path)))
    ...
    for varname, highlight_group in (
        ...
        set_tmux_environment(
            varname, powerline.renderer.hlstyle(**highlight)[2:-1])
    ...
    for varname, attr, group in (
            ...
            ('_POWERLINE_WINDOW_COLOR_BG', 'bg', 'window'),
            ('_POWERLINE_WINDOW_CURRENT_COLOR_BG', 'bg', 'window:current'),
    ):
        if attr == 'attrs':
            ...
            if powerline.common_config['term_truecolor']:
                set_tmux_environment(varname, '#{0:06x}'.format(
                    get_highlighting(group)[attr][1]))
            else:
                set_tmux_environment(varname, 'colour' +
                                     str(get_highlighting(group)[attr][0]))
...
def tmux_setup(pl, args):
    ...
    source_tmux_files(pl, args, tmux_version=tmux_version,
                      source_tmux_file=stf)
...
def create_powerline_logger(args):
    ...
    common_config = finish_common_config(
        get_preferred_output_encoding(), config['common'])
...
def uses(pl, args):
    ...
    for sh in (shell, 'shell') if shell else ('shell'):
        varname = template.format(
            shell=sh.upper(), component=component.upper())
