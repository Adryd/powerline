...
def last_status(pl, segment_info):
    ...
    if not segment_info['args'].last_exit_code:
        return [{'contents': str(segment_info['args'].last_exit_code), 'highlight_groups': ['exit_success']}]
    if segment_info['args'].last_exit_code == 127 or segment_info['args'].last_exit_code == 130:
        return[{'contents': str(segment_info['args'].last_exit_code), 'highlight_groups':['exit_moderate']}]
    else:
...
def continuation(pl, segment_info, omit_cmdsubst=True, right_align=False, renames={}):
    ...
    if right_align:
        ...
        ret[-1].update(width='auto', align='l',
                       highlight_groups=['continuation:current', 'continuation'])
