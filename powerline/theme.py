...
class Theme(object):
    def __init__(self,
        ...
        self.dividers_ns = theme_config['dividers_nospace']
        self.dividers_ns = dict((
            (key, dict((k, u(v))
                       for k, v in val.items()))
            for key, val in self.dividers_ns.items()
        ))
        try:
            self.cursor_space_multiplier = 1 - \
                (theme_config['cursor_space'] / 100)
        except KeyError:
                        ...
                        if not run_once:
                            if segment['startup']:
                                try:
                                    ...
                                    pl.error(
                                        'Exception during {0} startup: {1}', segment['name'], str(e))
                                    continue
                ...
                try:
                    ...
                    if width == 'auto' and segment['expand'] is None:
                        ...
                        if segment['expand'] is None:
                            self.pl.error(
                                'Align argument must be “r”, “l” or “c”, not “{0}”', align)
                    ...
                    try:
                        ...
                        self.pl.exception(
                            'Failed to compute segment contents: {0}', str(e))
                        segment['contents'] = safe_unicode(
                            segment.get('contents'))
